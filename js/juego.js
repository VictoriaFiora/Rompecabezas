...
function chequearSiGano(){
  for (var fila = 0; fila < 3; fila++) {
    for (var columna = 0; columna < 3; columna++) {
      var en_Orden = (grilla[fila][columna] == grilla_terminada[fila][columna])
      if (!en_Orden){
        return false;
      } 
    }
...
function intercambiarPosiciones(filaPos1, columnaPos1, filaPos2, columnaPos2){
  var n1 = grilla[filaPos1][columnaPos1];
  var n2 = grilla[filaPos2][columnaPos2];
  grilla[filaPos1][columnaPos1] = n2;
  grilla[filaPos2][columnaPos2] = n1;

  var ficha1 = document.getElementById(n1);
  var ficha2 = document.getElementById(n2);
  var padre = document.getElementById("juego");

  var cloneFicha1 = ficha1.cloneNode();
  var cloneFicha2 = ficha2.cloneNode();
  
  juego.replaceChild(cloneFicha1, ficha2);
  juego.replaceChild(cloneFicha2, ficha1);
}
...
function posicionValida(fila, columna){
  return (fila < 3 && columna < 3 && fila >= 0 && columna >= 0);
}
